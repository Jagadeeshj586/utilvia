...
exports.SIP_FAQS = exports.SIP_DISCLAIMER = exports.SIP_PRESETS = exports.SIP_LIMITS = exports.SIP_DEFAULTS = void 0;
exports.monthlyRateFromAnnual = monthlyRateFromAnnual;
exports.sipFutureValue = sipFutureValue;
exports.calculateStandardSIP = calculateStandardSIP;
exports.calculateStepUpSIP = calculateStepUpSIP;
exports.calculateSIP = calculateSIP;
exports.validateSipInput = validateSipInput;
exports.clampSipInput = clampSipInput;
exports.sipSummaryText = sipSummaryText;
exports.sipCopyText = sipCopyText;
const utils_1 = require("../utils");
exports.SIP_DEFAULTS = {
...
function sipFutureValue(monthlyInvestment, monthlyRate, months) {
...
function calculateStandardSIP(input) {
...
function calculateStepUpSIP(input) {
    const monthlyInvestment = input.monthlyInvestment;
    const annualReturn = input.annualReturn;
    const years = Math.round(input.years);
    const months = years * 12;
...
function validateSipInput(input) {
    ...
    if (!Number.isFinite(monthlyInvestment) || monthlyInvestment < exports.SIP_LIMITS.monthlyInvestment.min || monthlyInvestment > exports.SIP_LIMITS.monthlyInvestment.max) {
        errors.monthlyInvestment = `Enter an investment amount between ${(0, utils_1.formatINR)(exports.SIP_LIMITS.monthlyInvestment.min)} and ${(0, utils_1.formatINR)(exports.SIP_LIMITS.monthlyInvestment.max)}.`;
    }
    if (!Number.isFinite(annualReturn) || annualReturn < exports.SIP_LIMITS.annualReturn.min || annualReturn > exports.SIP_LIMITS.annualReturn.max) {
        errors.annualReturn = `Enter an expected return between ${exports.SIP_LIMITS.annualReturn.min}% and ${exports.SIP_LIMITS.annualReturn.max}%.`;
    }
    if (!Number.isFinite(years) || years < exports.SIP_LIMITS.years.min || years > exports.SIP_LIMITS.years.max) {
        errors.years = `Investment duration must be between ${exports.SIP_LIMITS.years.min} and ${exports.SIP_LIMITS.years.max} years.`;
    }
    if (stepUpEnabled) {
        const step = stepUpPercent ?? NaN;
        if (!Number.isFinite(step) || step < exports.SIP_LIMITS.stepUpPercent.min || step > exports.SIP_LIMITS.stepUpPercent.max) {
            errors.stepUpPercent = `Annual SIP increase must be between ${exports.SIP_LIMITS.stepUpPercent.min}% and ${exports.SIP_LIMITS.stepUpPercent.max}%.`;
        }
    }
    return errors;
}
function clampSipInput(input) {
    const clamp = (value, min, max) => Math.min(max, Math.max(min, value));
    return {
        monthlyInvestment: clamp(Number.isFinite(input.monthlyInvestment) ? input.monthlyInvestment : exports.SIP_DEFAULTS.monthlyInvestment, exports.SIP_LIMITS.monthlyInvestment.min, exports.SIP_LIMITS.monthlyInvestment.max),
        annualReturn: clamp(Number.isFinite(input.annualReturn) ? input.annualReturn : exports.SIP_DEFAULTS.annualReturn, exports.SIP_LIMITS.annualReturn.min, exports.SIP_LIMITS.annualReturn.max),
        years: Math.round(clamp(Number.isFinite(input.years) ? input.years : exports.SIP_DEFAULTS.years, exports.SIP_LIMITS.years.min, exports.SIP_LIMITS.years.max)),
        stepUpEnabled: Boolean(input.stepUpEnabled),
        stepUpPercent: clamp(Number.isFinite(input.stepUpPercent ?? exports.SIP_DEFAULTS.stepUpPercent) ? (input.stepUpPercent ?? exports.SIP_DEFAULTS.stepUpPercent) : exports.SIP_DEFAULTS.stepUpPercent, exports.SIP_LIMITS.stepUpPercent.min, exports.SIP_LIMITS.stepUpPercent.max),
    };
}
function sipSummaryText(result) {
    const monthly = (0, utils_1.formatINR)(result.monthlyInvestment);
    const invested = (0, utils_1.formatINR)(Math.round(result.totalInvested));
    const value = (0, utils_1.formatINR)(Math.round(result.futureValue));
...
function sipCopyText(result) {
    const lines = [
        "SIP Calculator",
        "",
        `Monthly Investment: ${(0, utils_1.formatINR)(result.monthlyInvestment)}`,
        `Expected Return: ${result.annualReturn}%`,
        `Duration: ${result.years} years`,
    ];
    if (result.mode === "step-up")
        lines.push(`Annual SIP Increase: ${result.stepUpPercent}%`);
    lines.push(`Total Invested: ${(0, utils_1.formatINR)(Math.round(result.totalInvested))}`, `Estimated Returns: ${(0, utils_1.formatINR)(Math.round(result.estimatedReturns))}`, `Estimated Value: ${(0, utils_1.formatINR)(Math.round(result.futureValue))}`, "", "Estimates only. Actual mutual fund returns may vary and are not guaranteed.");
    return lines.join("\n");
}
exports.SIP_PRESETS = [
    { id: "conservative", label: "Conservative", monthlyInvestment: 5000, annualReturn: 8, years: 10 },
    { id: "balanced", label: "Balanced", monthlyInvestment: 10000, annualReturn: 12, years: 15 },
    { id: "growth", label: "Growth", monthlyInvestment: 20000, annualReturn: 15, years: 20 },
];
exports.SIP_DISCLAIMER = "This calculator is for illustrative purposes only. The estimated returns are based on the assumed rate entered by the user and are not guaranteed. Actual mutual fund returns may vary depending on market conditions and other factors. This tool does not constitute investment advice.";
exports.SIP_FAQS = [
    {
